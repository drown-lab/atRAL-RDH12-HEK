...
contrast <- "RDH12_200_atRAL5hr.24h_recvr_vs_GFP_100_atRAL5hr.24h_recvr"
favorite_IDs <- c( "HSPA5", "CNBP", "ELOF1",
                 "RDH10", "RDH11", "RDH12", "RDH13", "RDH14", "TRIB3","ZNF319"
)  # example

res_longH <- data_results %>%
  transmute(
    name,
    ...
    sig   = padj < 0.054,
    direction = case_when(
      ratio >  log2(1.99) & sig ~ "Up",
      ratio < -log2(1.99) & sig ~ "Down",
      TRUE ~ "NS"
    )
  )
...
ggplot(res_longH, aes(x = ratio, y = -log10(padj))) +
  ...
  geom_vline(xintercept = c(-log2(1.98), log2(1.98)),
  ...
  theme_bw() +
  labs(
    title = paste("Volcano Heavy:", contrast),
    x = "log2 Fold Change",
    y = "-log10(adj. p-value)"
  ) +
  scale_color_manual(
