...
acute_contrast_labels <- c(
  ...
  "RDH12_control_atRAL5hr.24h_recvr_vs_GFP_control_atRAL5hr.24h_recvr" = "Veh RDH12 vs WT"
)
...
acute_col_labels <- c(
  ...
  "RDH12_control_atRAL5hr.24h_recvr_centered" = "Rcvr_RDH12 Veh",
  "GFP_control_atRAL5hr.24h_recvr_centered" = "Rcvr_WT Veh"
  
)

# Output files
out_file_pdf <- "Proteomic_Figs/enriched/Endo_acute_pathway_heatmap_centered_sigOnly_Lists_clusteredWithinPathway_wContrastAnno.pdf"
out_file_svg <- "Proteomic_Figs/enriched/Edndoacute_pathway_heatmap_centered_sigOnly_Lists_clusteredWithinPathway_wContrastAnno.svg"
out_file_csv <- "Proteomic_Figs/enriched/Endo_acute_pathway_heatmap_input_table_sigOnly_clusteredWithinPathway_wContrastAnno.csv"
...
EndoRet <- c("GJC1","APOO","TXNDC12","ANKLE2","RDH12","BNIP3","POM121","TMEM38B","SLC30A9",
                 "SEC63","PNPT1","IER3IP1","XXYLT1","SORL1","LPGAT1","ADGRG6","FBXO2","DOLK","OSBPL8","AMFR","C2CD2L","PIGX","ELAVL1"
...
pathway_tbl <- bind_rows(
  ...
  tibble(Gene = EndoRet,       Pathway = "EndoRet"
...
row_annot <- plot_df %>%
  transmute(
    ...
    `Veh RDH12 vs WT` =if_else(.data[[paste0(acute_contrasts[4], "_sigflag")]], "Yes", "No")
  ) %>%
  distinct() %>%
  as.data.frame()
...
pathway_colors <- list(
  ...
  `Veh RDH12 vs WT`= c("Yes" = "#51A3CC", "No" = "grey90")
)
