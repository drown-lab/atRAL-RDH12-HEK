...
Paper1_DEPresults_v1<-read.csv("Proteomic_output_txts/DEPresults_v1.csv")
...
miss_long <- Paper1_DEPresults_v1 %>%
  select(ID, starts_with("missclass_")) %>%
  pivot_longer(
...
dep_long_final3<-dep_long|>
  select( Gene, contrast, p.val, p.adj, ratio)|>
  unique()

#write a csv file of the volcano friednly table
write.csv(dep_long_final3, "Proteomic_output_txts/Volcanotable_v1.csv")
...
contrast <- "RDH12_100_atRAL5hr_vs_RDH12_control_atRAL5hr" #need to define contrast because normally have multiple contrast tested in DEP code
...
res_longH <- dep_long_final3 %>%
  filter(contrast =="RDH12_100_atRAL5hr_vs_RDH12_control_atRAL5hr")|> #need to call contrast of interest
  transmute(
    Gene,
