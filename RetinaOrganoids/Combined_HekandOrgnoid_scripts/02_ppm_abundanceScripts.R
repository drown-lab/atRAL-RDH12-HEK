...
combined_proteome2<-combined_proteome|>
  filter(!PG.MaxLFQ==0)|>
...
combined_proteome2 <- combined_proteome2 |>
...
combined_proteome3 <- combined_proteome2 |>
  group_by(CellType)|>
  mutate(
...
non_fractionated <- c("GFP", "RDH12")  # adjust to your exact CellType strings

combined_proteome3 <- combined_proteome2 %>%
  group_by(Protein.Group, Genes, CellType) %>%
  summarise(
    abundance = if (dplyr::first(CellType) %in% non_fractionated) {
      # pick ONE representative value per protein
      # (max is robust if there are tiny numeric differences)
      max(PG.MaxLFQ, na.rm = TRUE)
    } else {
      # real fractionated samples: add across fractions
      sum(PG.MaxLFQ, na.rm = TRUE)
    },
    n_fractions = dplyr::n_distinct(Fraction),
    .groups = "drop"
  ) %>%
  group_by(CellType) %>%
  mutate(
    ppm = abundance / sum(abundance, na.rm = TRUE) * 1e6,
    log10_ppm = log10(ppm)
  ) %>%
  ungroup()


combined_proteome4<-combined_proteome3|>
  filter(!CellType=="GFP")


target_id <- "RDH12"

target_ppm <- combined_proteome3 |>
...
ggplot(combined_proteome3, aes(x = ppm, fill = CellType)) +
  geom_histogram(
    bins = 60,
    position = "identity",
    alpha = 0.45,
...
ggplot(combined_proteome4, aes(x = ppm)) +
  geom_histogram(
    bins = 60,
    fill = "grey40",
    color = "white"
