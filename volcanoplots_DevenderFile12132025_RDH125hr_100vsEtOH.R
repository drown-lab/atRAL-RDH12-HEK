...
"Z:\data\Projects\Rams_Collab_RDH12experiments\atRAL_experiments\proteomics\DEPandEnrichment_DevenderAnalysis\FilesfromEmailon12132025\proteomics_comparison_RDH12_100uM_5hr_vs_control.xlsx"

###From 12-13-25 email

updateDE_RDH12_5h_100_vs_Control

###Devender Analysis for RDH12 5hr 100vsEtOH
updateDE_RDH12_5h_100_vs_Control<-read_xlsx("Z:/data/Projects/Rams_Collab_RDH12experiments/atRAL_experiments/proteomics/DEPandEnrichment_DevenderAnalysis/FilesfromEmailon12132025/proteomics_comparison_RDH12_100uM_5hr_vs_control.xlsx")

colnames(updateDE_RDH12_5h_100_vs_Control)
...
#DE_RDH12_5h_100_vs_Control_c2 <- DE_RDH12_5h_100_vs_Control %>%
# merge(RDH12_5h_100vsCtrl_v1, gene_mapping1, by= "ProteinID")

##GET GeneIDs since these files only have uniprot id
updateDE_RDH12_5h_100_vs_Control_c2 <- updateDE_RDH12_5h_100_vs_Control %>%
  left_join(gene_mapping1, by = "protein_ID")
colnames(updateDE_RDH12_5h_100_vs_Control_c2)

###Get/Clean GeneID
updateDE_RDH12_5h_100_vs_Control_c2<-updateDE_RDH12_5h_100_vs_Control_c2|>
  mutate(
...
contrast <- "RDH12 5hr 100 vs EtOH"
favorite_IDs <- c(
  "RDH10", "RDH11", "RDH12", "RDH13", "RDH14"
)  # example
##inpute table of interest
tableofinterest<-updateDE_RDH12_5h_100_vs_Control_c2   

colnames(tableofinterest)

tableofinterest <- tableofinterest |>
  mutate(log2FC = as.numeric(as.character(log2FC)))

class(tableofinterest$P.Value)
...
# helper: greedy "spread out" selection in PCA/volcano space
pick_spread <- function(df, n = 12, min_dist = 0.35) {
  # df must have columns: ratio, padj, GeneID
  df <- df |>
    mutate(y = -log10(padj)) |>
    arrange(padj)
  ...
  for (i in seq_len(nrow(df))) {
    ...
    if (length(keep) == 0) {
      ...
      dx <- df$ratio[i] - df$ratio[keep]
      dy <- df$y[i]     - df$y[keep]
      if (min(sqrt(dx^2 + dy^2)) >= min_dist) keep <- c(keep, i)
    }
  }
  df$GeneID[keep]
}

# choose 12 significant genes that are not on top of each other
top_spread <- pick_spread(res_longH, n = 12, min_dist = 0.35)

res_plot <- res_longH |>
  mutate(label = if_else(GeneID %in% c(top_spread, favorite_IDs), GeneID, NA_character_))
