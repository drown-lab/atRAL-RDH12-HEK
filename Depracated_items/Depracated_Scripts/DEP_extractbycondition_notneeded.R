...
#data_filt_acute <- filter_missval(data_se, thr = 0)

# Filter for proteins that are identified in 2 out of 3 replicates of at least one condition
#thr = 1 means "allow 1 missing per condition"
data_filt_acute <- filter_missval(data_se, thr = 1)

#data_filt_acute <- filter_missval(data_se, thr = 2)
plot_detect(data_filt_acute)

plot_missval(data_filt_acute)

plot_normalization(data_filt_acute)
...
## 2) extracting dataset types prior to imputation to save data
data_filt_acute <- data_filt[, colData(data_filt)$ExpType == "atRAL5hr"]
print(as.data.frame(colData(data_filt_acute)))

data_filt_recvry <- data_filt[, colData(data_filt)$ExpType == "atRAL5hr+24h_recvr"]
print(as.data.frame(colData(data_filt_recvry)))

data_filt_RDH12recvry <- data_filt_recvry[, colData(data_filt_recvry)$Genetype == "RDH12"]
print(as.data.frame(colData(data_filt_RDH12recvry)))


data_filt_GFPrecvry <- data_filt_recvry[, colData(data_filt_recvry)$Genetype == "GFP"]
print(as.data.frame(colData(data_filt_GFPrecvry)))
#=====================================================================================
## 2) Drop proteins that are ALL-NA in ANY condition
##    (prevents imputing structural absence, e.g., RDH12 in GFP)
mat  <- assay(data_filt_acute)
meta <- as.data.frame(colData(data_filt_acute))
...
data_filt_acute2 <- data_filt_acute[!drop_idx, ]

cat("Proteins in initial file:     ", nrow(data_se), "\n")
cat("Proteins after thr=1 filter:      ", nrow(data_filt_acute),  "\n")
cat("Proteins dropped (all-NA in a condition): ", sum(drop_idx), "\n")
cat("Proteins kept for imputation:     ", nrow(data_filt_acute2), "\n")

## ---------------------------------------------------------
data_imp_acute <- impute(data_filt_acute2, fun = "QRILC")
plot_imputation(data_se,data_filt_acute, data_filt_acute2, data_imp_acute)


####Checking
# BEFORE imputation
rdh12_raw <- assay(data_filt_acute)[rownames(data_filt_acute) == "RDH12", ]
rdh12_raw

rdh12_filt <- assay(data_filt_acute2)[rownames(data_filt_acute2) == "RDH12", ]
rdh12_filt
# AFTER imputation
rdh12_imp <- assay(data_imp_acute)[rownames(data_imp_acute) == "RDH12", ]
rdh12_imp


####Checking
# BEFORE imputation
RUSF1_raw <- assay(data_filt_acute)[rownames(data_filt_acute) == "RUSF1", ]
RUSF1_raw

# AFTER imputation
RUSF1_imp <- assay(data_imp_acute)[rownames(data_imp_acute) == "RUSF1", ]
RUSF1_imp
...
DEPallcontrasts_impuataioncorrect_v1<-data_results